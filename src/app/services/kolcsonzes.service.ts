...
//
//
  updateKeszlet(jid: number, id: number): Observable<Keszlet> {
    return this.http.put<Keszlet>('/api/keszlet', { id: jid, ugyfel: id }, this.httpOptions).pipe(
      first(),
      catchError(this.ErrorHandler)
    );
  }

  updateKeszlet1(id: number): Observable<Keszlet> {
    return this.http.put<Keszlet>('/api/keszlet', { id: id, ugyfel: null }, this.httpOptions).pipe(
      first(),
      catchError(this.ErrorHandler)
    );
  }
